...
public class GLTextureView
    ...
    protected void onAttachedToWindow() {
        ...
        if (LOG_ATTACH_DETACH) {
            logI(TAG, "onAttachedToWindow reattach =" + mDetached);
        }
    ...
    private class DefaultContextFactory implements EGLContextFactory {
        ...
        public void destroyContext(EGL10 egl, EGLDisplay display,
            ...
            if (!egl.eglDestroyContext(display, context)) {
                logI("DefaultContextFactory", "display:" + display + " context: " +
                        context);
                if (LOG_THREADS) {
                    logI("DefaultContextFactory", "tid=" + Thread.currentThread()
                            .getId());
                }
    ...
    private static class DefaultWindowSurfaceFactory implements EGLWindowSurfaceFactory {
        ...
        public EGLSurface createWindowSurface(EGL10 egl, EGLDisplay display,
            ...
            try {
                ...
                logE(TAG, "eglCreateWindowSurface"+e);
            }
            return result;
        }
    ...
    private static class EglHelper {
        ...
        public void start() {
            if (LOG_EGL) {
                logI("EglHelper", "start() tid=" + Thread.currentThread().getId());
            }
            /*
             * Get an EGL instance
             */
            ...
            if (LOG_EGL) {
                logI("EglHelper", "createContext " + mEglContext + " tid=" + Thread
                        .currentThread().getId());
            }
        ...
        public boolean createSurface() {
            if (LOG_EGL) {
                logI("EglHelper", "createSurface()  tid=" + Thread.currentThread()
                        .getId());
            }
            /*
             * Check preconditions.
            ...
            if (mEglSurface == null || mEglSurface == EGL10.EGL_NO_SURFACE) {
                ...
                if (error == EGL10.EGL_BAD_NATIVE_WINDOW) {
                    logI("EglHelper", "createWindowSurface returned " +
        ...
        public void destroySurface() {
            if (LOG_EGL) {
                logI("EglHelper", "destroySurface()  tid=" + Thread.currentThread()
                        .getId());
            }
            destroySurfaceImp();
        }
        ...
        public static void throwEglException(String function, int error) {
            ...
            if (LOG_THREADS) {
                logI("EglHelper", "throwEglException tid="
        ...
        public static void logEglErrorAsWarning(String tag, String function, int error) {
            logI(tag, formatEglError(function, error));
        }
    ...
    static class GLThread extends Thread {
        ...
        public void run() {
            ...
            if (LOG_THREADS) {
                logI("GLThread", "starting tid=" + getId());
            }
        ...
        private void guardedRun() throws InterruptedException {
            ...
            try {
                ...
                while (true) {
                    synchronized (sGLThreadManager) {
                        while (true) {
                            ...
                            if (mPaused != mRequestPaused) {
                                ...
                                if (LOG_PAUSE_RESUME) {
                                    logI("GLThread", "mPaused is now " + mPaused + " " +
                                            "tid=" + getId());
                                }
                            }
                            ...
                            if (mShouldReleaseEglContext) {
                                if (LOG_SURFACE) {
                                    logI("GLThread", "releasing EGL context because " +
                                            "asked to tid=" + getId());
                                }
                            ...
                            if (pausing && mHaveEglSurface) {
                                if (LOG_SURFACE) {
                                    logI("GLThread", "releasing EGL surface because " +
                                            "paused tid=" + getId());
                                }
                                stopEglSurfaceLocked();
                            }
                            ...
                            if (pausing && mHaveEglContext) {
                                ...
                                if (!preserveEglContextOnPause || sGLThreadManager
                                    ...
                                    stopEglContextLocked();
                                    if (LOG_SURFACE) {
                                        logI("GLThread", "releasing EGL context " +
                                                "because paused tid=" + getId());
                                    }
                                }
                            }
                            ...
                            if (pausing) {
                                if (sGLThreadManager.shouldTerminateEGLWhenPausing()) {
                                    mEglHelper.finish();
                                    if (LOG_SURFACE) {
                                        logI("GLThread", "terminating EGL because " +
                                                "paused tid=" + getId());
                                    }
                                }
                            }
                            ...
                            if ((!mHasSurface) && (!mWaitingForSurface)) {
                                if (LOG_SURFACE) {
                                    logI("GLThread", "noticed surfaceView surface " +
                                            "lost tid=" + getId());
                                }
                                if (mHaveEglSurface) {
                                    stopEglSurfaceLocked();
                            ...
                            if (doRenderNotification) {
                                if (LOG_SURFACE) {
                                    logI("GLThread", "sending render notification " +
                                            "tid=" + getId());
                                }
                            ...
                            // wait().
                            if (LOG_THREADS) {
                                logI("GLThread", "waiting tid=" + getId()
                    ...
                    if (createEglSurface) {
                        if (LOG_SURFACE) {
                            logI("GLThread", "egl createSurface");
                        }
                    ...
                    switch (swapError) {
                        ...
                        case EGL11.EGL_CONTEXT_LOST:
                            if (LOG_SURFACE) {
                                logI("GLThread", "egl context lost tid=" + getId());
                            }
                            lostEglContext = true;
                            break;
                        default:
        ...
        public void surfaceCreated() {
            synchronized (sGLThreadManager) {
                if (LOG_THREADS) {
                    logI("GLThread", "surfaceCreated tid=" + getId());
                }
                mHasSurface = true;
                sGLThreadManager.notifyAll();
        ...
        public void surfaceDestroyed() {
            synchronized (sGLThreadManager) {
                if (LOG_THREADS) {
                    logI("GLThread", "surfaceDestroyed tid=" + getId());
                }
                mHasSurface = false;
                sGLThreadManager.notifyAll();
                while ((!mWaitingForSurface) && (!mExited)) {
        ...
        public void onPause() {
            synchronized (sGLThreadManager) {
                if (LOG_PAUSE_RESUME) {
                    logI("GLThread", "onPause tid=" + getId());
                }
                mRequestPaused = true;
                sGLThreadManager.notifyAll();
        ...
        public void onResume() {
            synchronized (sGLThreadManager) {
                if (LOG_PAUSE_RESUME) {
                    logI("GLThread", "onResume tid=" + getId());
                }
        ...
        public void onWindowResize(int w, int h) {
            synchronized (sGLThreadManager) {
                ...
                while (!mExited && !mPaused && !mRenderComplete
                        && ableToDraw()) {
                    if (LOG_SURFACE) {
                        logI("Main thread", "onWindowResize waiting for render complete from " +
                                "tid=" + getId());
                    }
                    try {
                        sGLThreadManager.wait();
    ...
    static class LogWriter extends Writer {
        ...
        private void flushBuilder() {
            if (mBuilder.length() > 0) {
                logI("GLTextureView", mBuilder.toString());
    ...
    private static class GLThreadManager {
        ...
        public synchronized void checkGLDriver(GL10 gl) {
            if (!mGLESDriverCheckComplete) {
                ...
                if (LOG_SURFACE) {
                    logI(TAG, "checkGLDriver renderer = \"" + renderer + "\" " +
    ...
    public static void setLogger(ILogger logger) {
        mLogRef = new WeakReference<>(logger);
    }

    public static void logI(String log){
        if(mLogRef!=null&& mLogRef.get()!=null){
            mLogRef.get().i(TAG,log);
        }
    }

    public static void logE(String log){
        if(mLogRef!=null&& mLogRef.get()!=null){
            mLogRef.get().e(TAG,log);
        }
    }
